...
In the iframe setup, the iframe should be rendered with a query parameter `iframe=1`. An optional query parameter `correlationId` can be used to identify several instances of our form iframes. An optional query parameter `autoHeight=1` can be used to enable automatic height adjustment of the iframe (see [Iframe height](#iframe-height) below). To use this setup the parent origins must be allowed in our form configuration to prevent phishing.

If using Content Security Policy (CSP) the form origin should be allowed in `frame-src`:
- https://form.pingwire.io on production
- https://form.staging.pingwire.io on staging

If using the `sandbox` attribute on the `<iframe>` element, the following sandbox directives are required:
- `allow-scripts`: the iframe needs to run JavaScript
- `allow-same-origin`: the iframe needs to make cross-origin API calls with its correct origin so that the backend can verify the requests come from an expected origin (CORS checks)

> **Note:** Combining `allow-scripts` and `allow-same-origin` is generally discouraged because it can, in some cases, allow the iframe to remove its own sandbox restrictions using scripts (e.g. via `document.domain` manipulation). However, this attack vector only applies when the iframe and the parent page share the same origin. Since our form is always served from a dedicated origin (`form.pingwire.io` or `form.staging.pingwire.io`) which will differ from your application's origin, this risk does not apply here.
...
|`IFRAME_RESIZE`|Message sent by the iframe to give the new height of the iframe. Only emitted when the `autoHeight=1` query parameter is passed.| `height`: height of the iframe (in px)|

#### Iframe height

The recommended iframe height depends on whether the `autoHeight` query parameter is used:

- **Without `autoHeight`**: Set a fixed height of `100dvh` on the `<iframe>` element so the form fills the full viewport height.
- **With `autoHeight=1`**: Do not set a fixed height on the `<iframe>`. Instead, listen for `IFRAME_RESIZE` messages and update the iframe height dynamically from `data.payload.height`. This is useful when the form is embedded inline in a page and should grow or shrink to match its content.

The `IFRAME_RESIZE` event is **only emitted** when the `autoHeight=1` query parameter is present.
